#!/usr/bin/env python3
"""
Simple Technical Scanner - MSFT Specific
This is the original ML-optimized scanner that performed well on MSFT
...
def simple_technical_msft(train_data: pd.DataFrame, test_data: pd.DataFrame, **kwargs) -> pd.DataFrame:
    """
    MSFT-specific technical scanner (original ML-optimized version)
    
    This performed well in testing with +2.05% return
    Keeping original parameters and weights that work for MSFT
    """
    ...
    df = calculate_msft_indicators(combined_data, params)
    
    # Get dynamic thresholds
    thresholds = get_msft_dynamic_thresholds(df, params)
    ...
    for i in range(max(params.values()) + 15, len(test_df)):
        signal_score = calculate_msft_signal_score(
...
def calculate_msft_indicators(df: pd.DataFrame, params: Dict) -> pd.DataFrame:
...
def get_msft_dynamic_thresholds(df: pd.DataFrame, params: Dict) -> Dict:
...
def calculate_msft_signal_score(df: pd.DataFrame, idx: int, params: Dict, 
                               weights: Dict, thresholds: Dict) -> float:
    """Calculate signal score for MSFT"""
